...
for image in $images; do
    ...
    for tag in $tags; do
        ...
        trivy image --format template --template "@/usr/bin/html.tpl" -o "$output_dir/${image_file_name}-${tag}-report.html" $image_full
        
        echo "Report saved for $image_full at $output_dir/${image_file_name}-${tag}-report.html"
    done
done
